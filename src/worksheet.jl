...
function JSONWorksheet(xlsxpath, sheet, arr; 
    ...
    pointer = map(el ->  startswith(el, JSONPointer.TOKEN_PREFIX) ? Pointer(el) : 
                                                                    Pointer(JSONPointer.TOKEN_PREFIX * el) 
                    , arr[1, :])
...
function create_by_pointer(::Type{T}, arr::Array) where T <: AbstractDict
    template = T{String, Any}()
    if length(arr) > 1
        @inbounds for p in arr
            p2 = Pointer{Any}(p.token)
            template[p2] = JSONPointer.null_value(p)
        end
    end
    return template
end
...
function squeezerow_to_jsonarray(data::Array{T, 2}, pointers, delim) where T
    arr_pointer = map(p -> begin 
                U = Vector{eltype(p)}; Pointer{U}(p.token)
        end, pointers)
